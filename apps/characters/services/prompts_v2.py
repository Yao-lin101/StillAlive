...
FINDINGS_PROGRESS_RULE_TEMPLATE = """- **时间线性挖掘约束**：你之前的发现已截止于北京时间 **{last_end_time}**。**绝对禁止**对该时间点之前的数据进行任何形式的挖掘。你的任务是发现 **{last_end_time} 之后**的新亮点。如果没有新亮点，请保持 `new_slots` 为空。"""
...
**执行要求**：
1. **时间线性增长**：**强制约束**：新发现的时间范围必须在【已分析记录】的最后结束时间之后。禁止对已经产生过发现的时间段进行重复挖掘或多重挖掘。
2. **增量去重**：请参考下方的 `existing_finding_keys`，不要重复已有的发现点（即使换了角度也不行）。
3. **输出要求**：如果没有符合时间线性要求且足够有趣的新发现，请在 `new_slots` 中返回空数组。
4. **输出格式**：
   - `overall`: 今日核心发现的汇总（需整合历史发现与本次新发现）。
   - `new_slots`: 此次新增的有趣细节。时间必须严格递增。
...
FINDINGS_EXISTING_SLOTS_TEMPLATE = """
# 已有发现记录（已归档，时间点已被占据）
以下是之前已确定的有趣发现，请在此基础之上，**严格分析 {last_end_time} 之后**的新数据：
<existing_locked_findings>
{locked_slots_json}
</existing_locked_findings>
