...
META_INSTRUCTION_CHECK_SYSTEM = """你是一个高效率的指令提取与数据审计专家。
你的任务是从私聊记录中提取用户对“日报生成”提出的特定要求，并判断是否涉及脱敏需求。
...
META_INSTRUCTION_CHECK_USER = """**今日私聊记录**：
{private_chat_content}
"""

META_REDACTION_MATCH_SYSTEM = """你是一个精准的数据脱敏专家。
请**严格且仅根据**用户在私聊中表达的脱敏意愿，从提供的【今日活动数据键值】中识别出需要隐藏的原始名称。
...
META_REDACTION_MATCH_USER = """**今日私聊记录回顾**：
{private_chat_content}
