...
3. 数据局限性与推理指南：
   - **巨额时长陷阱**：系统**仅在切换应用时**产生记录。若出现离谱的单次时长，**需结合应用类型及其它平均使用时长综合判断**：对于游戏、长视频等应用，可能确实是长时间连续使用；但对于普通工具、社交类应用，大概率是短暂使用后便**息屏离开、睡着或去忙现实事务了**，直到几小时后才再次操作。
...
2. **数据局限性与推理指南**：
   ...
   - **后台驻留盲区**：系统只记录前台。像网易云音乐等音频应用，如果只有 `[0.6]` 这样的短暂记录，大概率是点开播放后就切后台/息屏听歌了，不能说"只听了半分钟"。
   - **步数累计陷阱**：步数是**当天的累计总值**。小时级数据代表"截至该时刻的总步数"，**绝对禁止**将各小时步数相加进行计算！
   - **聊天记录陷阱**：提供的聊天记录和消息详情**绝对禁止**以“用户：... / 回复：...”的原始对话格式生硬罗列，或出现“聊天记录揭示了”、“从聊天记录可以看出”等出戏话语。请将消息内容**转化为自然的互动记忆或观察结论**，必须完全融入你的角色口吻！
3. **时区**：所有时间均为北京时间。
"""
