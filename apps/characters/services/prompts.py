...
# 1. 原始指令提取提示词（保留兼容性）
META_INSTRUCTION_EXTRACTION_PROMPT = """你是一个高效率的指令提取与数据审计专家。
请从以下【今日私聊记录】中，识别出用户对“日报生成”提出的要求，并结合【今日活动数据键值】识别出需要脱敏的内容。
...
**任务 2：识别脱敏项**
- 如果用户要求隐藏、保密、不要提到某些活动或应用。
- **严格限制**：仅能从【今日活动数据键值】列表中挑选需要脱敏的词条。
- **严禁虚构**：不要脱敏那些仅出现在【今日私聊记录】中但未出现在【今日活动数据键值】中的词汇。
- **输出格式**：返回一个包含需要脱敏的原始名称的**列表**（List）。

**输出格式**：
必须返回一个合法的 JSON 对象，格式如下：
{{
  "instructions": ["要求1", "要求2"] 或 [],
  "redactions": ["原始名称A", "原始名称B"] 或 [],
  "has_any": true 或 false
}}
